...
class Grille():
  ...
  def __str__(self): #Pour afficher une grille simple
  ...
  def placeGrille(self,t,p): #Place un symbole sur la grille. Renvoie faux si la case est non vide. Si le mouvement permet de gagner, change la valeur de winner de None au symbole du joueur.
    if self.grille[p]==" ":
      self.grille[p]=t
      if self.win():
        self.winner = t
  ...
  def win(self): #Vérifie que la grille actuelle a une victoire et renvoie vrai ou faux.
...
class SmartCpu(Joueur):
  ...
  def smart(self,etat,joueur): #etat est un état du jeu, joueur est le symbole de celui qui doit jouer
    ...
    for p in etat.legit():
      #à compléter : on veut placer le symbole, récupérer le résultat en fonction du nouvel état (récursivement), puis faire un backtrack (attention : quand on place quelque chose, une autre variable change...)
      #On met alors à jour le résultat avec l'emplacement.
      #Enfin, on met à jour le score s'il est meilleur, dans le cas considéré (on maximise son score, on minimise celui de l'autre).
      #Courage !
      pass
